...
verify(_Base, Req, NodeMsg) ->
    ...
    Ledger =
        hb_ao:get(
            <<"ledger">>,
            Req,
            hb_opts:get(ao_payment_ledger, undefined, NodeMsg),
            NodeMsg
        ),
    DepositAddress =
        hb_ao:get(
    ...
    MessageID =
        hb_ao:get(
            <<"message-id">>,
            Req,
            hb_ao:get(<<"id">>, Req, undefined, NodeMsg),
            NodeMsg
        ),
    ...
    of
        true ->
            {error, #{
                <<"status">> => 400,
                <<"body">> =>
                    <<"Missing token, ledger, deposit-address, message-id, "
                        "slot, sender, or quantity.">>
            }};
        false ->
    ...
    case Found of
        ...
        false ->
            {error, #{
                <<"status">> => 402,
                <<"body">> =>
                    <<"AO schedule did not contain the expected Transfer assignment.">>
            }}
    end.
    ...
    case {Debit, Credit} of
        ...
        _ ->
            {error, #{
                <<"status">> => 402,
                <<"body">> =>
                    <<"AO result did not contain the expected Debit-Notice "
                        "and Credit-Notice.">>
            }}
    end.
        ...
        fun(Msg) ->
            ...
            hb_maps:get(<<"Target">>, Msg, undefined, NodeMsg) =:= Target
                ...
                andalso maps:fold(
                    fun(Name, Value, Acc) ->
                        Acc andalso
                            hb_util:bin(tag_value(Tags, Name, NodeMsg)) =:= Value
                    end,
                    true,
                    RequiredTags
                )
...
credit_to_payment(CreditMsg, Expected, NodeMsg) ->
    ...
    Sender = hb_maps:get(<<"sender">>, Expected, undefined, NodeMsg),
    RequestedRecipient =
        hb_maps:get(<<"requested-recipient">>, Expected, undefined, NodeMsg),
    Recipient =
    ...
    case RequestedRecipient =:= undefined orelse RequestedRecipient =:= Recipient of
        true ->
            {ok, #{
                ...
                <<"message-id">> =>
                    hb_maps:get(<<"message-id">>, Expected, undefined, NodeMsg),
                <<"ledger">> => hb_maps:get(<<"ledger">>, Expected, undefined, NodeMsg),
                <<"sender">> => Sender,
                <<"recipient">> => Recipient,
                <<"quantity">> =>
                    hb_maps:get(<<"quantity">>, Expected, undefined, NodeMsg),
...
import_payment(Payment, NodeMsg) ->
    Wallet =
        case hb_opts:get(priv_wallet, not_found, NodeMsg) of
            not_found ->
                hb:wallet(
                    hb_opts:get(priv_key_location, <<"hyperbeam-key.json">>, NodeMsg)
                );
            FoundWallet ->
                FoundWallet
        end,
    ...
    CreditNotice =
        hb_message:commit(
            #{
                ...
                <<"from-process">> =>
                    hb_maps:get(<<"token">>, Payment, undefined, NodeMsg),
                <<"recipient">> =>
                    hb_maps:get(<<"recipient">>, Payment, undefined, NodeMsg),
                <<"quantity">> =>
                    hb_util:int(hb_maps:get(<<"quantity">>, Payment, undefined, NodeMsg)),
                <<"sender">> =>
                    hb_maps:get(<<"sender">>, Payment, undefined, NodeMsg),
                <<"ao-payment-id">> =>
                    hb_maps:get(<<"message-id">>, Payment, undefined, NodeMsg)
            },
            Opts
        ),
...
verify_schedule_matches_expected_transfer_test() ->
    ...
    ?assertMatch(
        {error, #{ <<"status">> := 402 }},
        verify_schedule(Schedule, Expected#{ <<"quantity">> => <<"2">> }, #{})
    ).
